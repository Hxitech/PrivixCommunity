...
if [ "$DEBUG" = true ]; then
  ...
  if [[ -n "$TARGET" ]]; then
    ...
    EFFECTIVE_TARGET="aarch64-apple-darwin"
    echo -e "  ${GRAY}构建 ARM64 版本...${RESET}"
    npm run tauri build -- --target "$EFFECTIVE_TARGET"
  else
    npm run tauri build
  fi
fi
...
if [ "$DEBUG" = true ]; then
  if [[ -n "$EFFECTIVE_TARGET" ]]; then
    echo -e "  可执行文件: src-tauri/target/$EFFECTIVE_TARGET/debug/clawpanel"
  else
    echo -e "  可执行文件: src-tauri/target/debug/clawpanel"
  fi
else
  if [[ -n "$EFFECTIVE_TARGET" ]]; then
    BUNDLE_DIR="src-tauri/target/$EFFECTIVE_TARGET/release/bundle"
  else
