...
fn hermes_gateway_url_is_local() -> bool {
    let url = hermes_gateway_url().trim().to_ascii_lowercase();
...
/// 将面板中的 provider key 归一化为 Hermes 使用的环境变量族。
/// Hermes 主要通过 .env 中的 key/base_url 自动识别供应商。
fn hermes_env_provider(provider: &str, base_url: Option<&str>) -> &'static str {
    let normalized = base_url.unwrap_or("").trim().trim_end_matches('/').to_ascii_lowercase();
    match provider {
        "anthropic" => "anthropic",
        "openrouter" => "openrouter",
        "google" | "google-genai" | "gemini" => "gemini",
        "deepseek" => "deepseek",
        "xai" => "xai",
        "aliyun" | "alibaba" | "dashscope" => "alibaba",
        "zhipu" | "zai" | "glm" | "z-ai" | "z.ai" => "zai",
        "moonshot" | "kimi" | "kimi-coding" | "kimi-for-coding" => {
            if normalized.contains("moonshot.cn") {
                "kimi-coding-cn"
            } else {
                "kimi-coding"
            }
        ...
        "minimax" | "minimax-cn" => {
            if provider == "minimax-cn"
                || normalized.contains("api.minimax.chat")
...
/// 基于 base_url 为 UI 推断更友好的 provider key。
fn hermes_provider_from_base_url(base_url: &str) -> Option<&'static str> {
    ...
    if normalized.contains("api.kimi.com") {
        return Some("kimi-coding");
    }
    if normalized.contains("api.moonshot.ai") || normalized.contains("api.moonshot.cn") {
    ...
    if normalized.contains("dashscope.aliyuncs.com") || normalized.contains("dashscope-intl.aliyuncs.com") {
        return Some("aliyun");
    }
...
/// 将 Hermes 原生 provider / alias 规范化为 Privix UI 使用的 provider key。
fn hermes_ui_provider(provider: &str, base_url: Option<&str>) -> String {
    match provider.trim().to_ascii_lowercase().as_str() {
        "" => hermes_provider_from_base_url(base_url.unwrap_or("")).unwrap_or("").to_string(),
        "anthropic" => "anthropic".into(),
        "openrouter" => "openrouter".into(),
        "google" | "google-genai" | "gemini" => "google".into(),
        "deepseek" => "deepseek".into(),
        "xai" => "xai".into(),
        "aliyun" | "alibaba" | "dashscope" => "aliyun".into(),
        "zhipu" | "zai" | "glm" | "z-ai" | "z.ai" => "zhipu".into(),
        "moonshot" | "kimi" | "kimi-coding" | "kimi-coding-cn" | "kimi-for-coding" => "moonshot".into(),
        "minimax" | "minimax-cn" => "minimax".into(),
        other => other.to_string(),
    }
}

fn hermes_api_key_env_key(env_provider: &str) -> &'static str {
    match env_provider {
...
fn hermes_base_url_env_key(env_provider: &str) -> Option<&'static str> {
    match env_provider {
        ...
        _ => Some("OPENAI_BASE_URL"),
    }
}

fn hermes_provider_from_env(env_values: &std::collections::HashMap<String, String>) -> Option<&'static str> {
    if env_values.contains_key("KIMI_API_KEY") || env_values.contains_key("KIMI_BASE_URL") {
        return Some("moonshot");
    }
    if env_values.contains_key("KIMI_CN_API_KEY") {
        return Some("moonshot");
    }
    if env_values.contains_key("MINIMAX_CN_API_KEY") || env_values.contains_key("MINIMAX_CN_BASE_URL") {
        return Some("minimax");
    }
    if env_values.contains_key("MINIMAX_API_KEY") || env_values.contains_key("MINIMAX_BASE_URL") {
        return Some("minimax");
    }
    if env_values.contains_key("GOOGLE_API_KEY")
        || env_values.contains_key("GEMINI_API_KEY")
        || env_values.contains_key("GEMINI_BASE_URL")
    {
        return Some("google");
    }
    if env_values.contains_key("GLM_API_KEY")
        || env_values.contains_key("ZAI_API_KEY")
        || env_values.contains_key("Z_AI_API_KEY")
        || env_values.contains_key("GLM_BASE_URL")
    {
        return Some("zhipu");
    }
    if env_values.contains_key("DASHSCOPE_API_KEY") || env_values.contains_key("DASHSCOPE_BASE_URL") {
        return Some("aliyun");
    }
    if env_values.contains_key("DEEPSEEK_API_KEY") || env_values.contains_key("DEEPSEEK_BASE_URL") {
        return Some("deepseek");
    }
    if env_values.contains_key("XAI_API_KEY") || env_values.contains_key("XAI_BASE_URL") {
        return Some("xai");
    }
    if env_values.contains_key("ANTHROPIC_API_KEY") || env_values.contains_key("ANTHROPIC_BASE_URL") {
        return Some("anthropic");
    }
    if env_values.contains_key("OPENROUTER_API_KEY") || env_values.contains_key("OPENROUTER_BASE_URL") {
        return Some("openrouter");
    }
    if env_values.contains_key("OPENAI_API_KEY") || env_values.contains_key("OPENAI_BASE_URL") {
        return Some("openai");
    }
    None
}

fn hermes_read_api_key_from_env(
    env_values: &std::collections::HashMap<String, String>,
    env_provider: &str,
) -> String {
    match env_provider {
        "anthropic" => env_values.get("ANTHROPIC_API_KEY").cloned().unwrap_or_default(),
        "openrouter" => env_values.get("OPENROUTER_API_KEY").cloned().unwrap_or_default(),
        "gemini" => env_values
            .get("GOOGLE_API_KEY")
            .cloned()
            .or_else(|| env_values.get("GEMINI_API_KEY").cloned())
            .unwrap_or_default(),
        "zai" => env_values
            .get("GLM_API_KEY")
            .cloned()
            .or_else(|| env_values.get("ZAI_API_KEY").cloned())
            .or_else(|| env_values.get("Z_AI_API_KEY").cloned())
            .unwrap_or_default(),
        "kimi-coding" => env_values.get("KIMI_API_KEY").cloned().unwrap_or_default(),
        "kimi-coding-cn" => env_values.get("KIMI_CN_API_KEY").cloned().unwrap_or_default(),
        "minimax-cn" => env_values.get("MINIMAX_CN_API_KEY").cloned().unwrap_or_default(),
        "minimax" => env_values.get("MINIMAX_API_KEY").cloned().unwrap_or_default(),
        "deepseek" => env_values.get("DEEPSEEK_API_KEY").cloned().unwrap_or_default(),
        "xai" => env_values.get("XAI_API_KEY").cloned().unwrap_or_default(),
        "alibaba" => env_values.get("DASHSCOPE_API_KEY").cloned().unwrap_or_default(),
        _ => env_values
            .get("OPENAI_API_KEY")
            .cloned()
            .or_else(|| env_values.get("KIMI_API_KEY").cloned())
            .or_else(|| env_values.get("GOOGLE_API_KEY").cloned())
            .or_else(|| env_values.get("GEMINI_API_KEY").cloned())
            .or_else(|| env_values.get("DEEPSEEK_API_KEY").cloned())
            .or_else(|| env_values.get("XAI_API_KEY").cloned())
            .or_else(|| env_values.get("DASHSCOPE_API_KEY").cloned())
            .or_else(|| env_values.get("GLM_API_KEY").cloned())
            .or_else(|| env_values.get("MINIMAX_CN_API_KEY").cloned())
            .or_else(|| env_values.get("MINIMAX_API_KEY").cloned())
            .or_else(|| env_values.get("ANTHROPIC_API_KEY").cloned())
            .or_else(|| env_values.get("OPENROUTER_API_KEY").cloned())
            .unwrap_or_default(),
    }
}

/// Privix 管理的 uv 二进制存放路径
...
async fn ensure_uv(app: &tauri::AppHandle) -> Result<String, String> {
    ...
    let client = super::build_http_client(std::time::Duration::from_secs(300), Some("Privix"))
...
fn extract_uv_tar_gz(data: &[u8], dest: &std::path::Path) -> Result<(), String> {
    ...
    for entry in archive.entries().map_err(|e: std::io::Error| format!("tar 解析失败: {e}"))? {
        let mut entry: tar::Entry<_> = entry.map_err(|e| format!("tar 条目读取失败: {e}"))?;
        let path = entry
            .path()
...
pub async fn configure_hermes(
    ...
    let env_provider = hermes_env_provider(&provider, base_url.as_deref());
    ...
    let config_content = if config_path.exists() {
        ...
        // 首次创建：生成完整的基线配置
        format!(
            r#"# Hermes Agent configuration (managed by Privix)
model:
  default: {model_str}
...
"#
    ...
    let env_key = hermes_api_key_env_key(env_provider);
    // Privix 管理的 key 列表（更新时覆盖，其他 key 保留）
    let managed_keys: Vec<&str> = vec![
        ...
        "OPENAI_BASE_URL", "OPENROUTER_BASE_URL", "ANTHROPIC_BASE_URL", "GEMINI_BASE_URL", "GLM_BASE_URL",
        "KIMI_BASE_URL", "MINIMAX_BASE_URL", "MINIMAX_CN_BASE_URL",
        "DEEPSEEK_BASE_URL", "XAI_BASE_URL", "DASHSCOPE_BASE_URL",
    ...
    if let Some(ref url) = base_url {
        if !url.trim().is_empty() {
            if let Some(url_key) = hermes_base_url_env_key(env_provider) {
    ...
    let should_restart_gateway = hermes_gateway_url_is_local() && gateway_quick_health_check().await;
    if should_restart_gateway {
        let _ = hermes_gateway_action(app.clone(), "stop".into()).await;
        match hermes_gateway_action(app, "start".into()).await {
            Ok(_) => Ok("配置已保存，并已自动重启 Gateway".into()),
            Err(e) => Ok(format!("配置已保存，但 Gateway 自动重启失败: {e}")),
        }
    } else {
        Ok("配置已保存".into())
    }
...
/// 规范化 Hermes config.yaml 中会导致运行时崩溃的空 map section。
///
/// 目前主要修复 `delegation:` 被写成 YAML null 的情况；Hermes 在
/// delegate_task 路径里会把它当作 dict 使用，null 会触发
/// `'NoneType' object has no attribute 'get'` 并陷入重试循环。
fn normalize_hermes_config_yaml_content(existing: &str) -> String {
    normalize_top_level_null_map(existing, "delegation")
}

fn normalize_top_level_null_map(existing: &str, key: &str) -> String {
    ...
    while i < lines.len() {
        ...
        if is_top_level && trimmed == format!("{key}:") {
            let mut j = i + 1;
            while j < lines.len() && lines[j].trim().is_empty() {
                j += 1;
            }
            let next_is_top_level_or_eof =
                j >= lines.len() || (!lines[j].starts_with("  ") && !lines[j].starts_with('\t'));
            if next_is_top_level_or_eof {
...
fn sanitize_hermes_config_file_if_needed(
    ...
    let normalized = normalize_hermes_config_yaml_content(&existing);
    if normalized == existing {
        return Ok(false);
    }
    std::fs::write(config_path, &normalized).map_err(|e| format!("修正 config.yaml 失败: {e}"))?;
    Ok(true)
}
...
fn merge_hermes_config_yaml(
    ...
    normalize_hermes_config_yaml_content(&final_content)
}
...
pub async fn hermes_read_config() -> Result<Value, String> {
    ...
    let config_path = home.join("config.yaml");
    let env_path = home.join(".env");
    let _ = sanitize_hermes_config_file_if_needed(&config_path);
    ...
    for line in config_raw.lines() {
        let trimmed = line.trim();
        let is_top_level = !line.starts_with("  ") && !line.starts_with('\t');
        if is_top_level && trimmed.starts_with("model:") {
            in_model = true;
            // model: "xxx" 单行格式
    ...
    if provider.is_empty() {
        if let Some(inferred_provider) = hermes_provider_from_base_url(&base_url) {
            provider = inferred_provider.to_string();
        } else if let Some(inferred_provider) = hermes_provider_from_env(&env_values) {
            provider = inferred_provider.to_string();
        }
    }

    let configured_env_provider = hermes_env_provider(&provider, Some(&base_url));
    if base_url.is_empty() {
        base_url = if let Some(url_key) = hermes_base_url_env_key(configured_env_provider) {
            env_values.get(url_key).cloned().unwrap_or_default()
        } else {
            String::new()
        };
        if base_url.is_empty() {
            base_url = env_values.get("OPENROUTER_BASE_URL")
                .cloned()
                .or_else(|| env_values.get("OPENAI_BASE_URL").cloned())
                .or_else(|| env_values.get("KIMI_BASE_URL").cloned())
                .or_else(|| env_values.get("GEMINI_BASE_URL").cloned())
                .or_else(|| env_values.get("GLM_BASE_URL").cloned())
                .or_else(|| env_values.get("MINIMAX_CN_BASE_URL").cloned())
                .or_else(|| env_values.get("MINIMAX_BASE_URL").cloned())
                .or_else(|| env_values.get("DEEPSEEK_BASE_URL").cloned())
                .or_else(|| env_values.get("XAI_BASE_URL").cloned())
                .or_else(|| env_values.get("DASHSCOPE_BASE_URL").cloned())
    ...
    provider = hermes_ui_provider(&provider, Some(&base_url));

    let env_provider = hermes_env_provider(&provider, Some(&base_url));
    let api_key = hermes_read_api_key_from_env(&env_values, env_provider);
...
pub async fn hermes_health_check() -> Result<Value, String> {
    ...
    let client = super::build_http_client(std::time::Duration::from_secs(5), Some("Privix"))
        .map_err(|e| format!("HTTP 客户端创建失败: {e}"))?;
...
pub async fn hermes_set_gateway_url(url: Option<String>) -> Result<String, String> {
    ...
    // 确保 hermes 对象存在
    if !config.get("hermes").map_or(false, |v: &Value| v.is_object()) {
...
pub async fn hermes_api_proxy(
    ...
    let client = super::build_http_client(timeout, Some("Privix"))
        .map_err(|e| format!("HTTP 客户端创建失败: {e}"))?;
...
pub async fn hermes_agent_run(
    ...
    let client = super::build_http_client(std::time::Duration::from_secs(10), Some("Privix"))
        .map_err(|e| format!("HTTP 客户端创建失败: {e}"))?;
    ...
    let sse_client = super::build_http_client(std::time::Duration::from_secs(300), Some("Privix"))
...
mod tests {
    ...
    #[test]
    fn minimax_cn_uses_native_env_family() {
        assert_eq!(hermes_env_provider("minimax", Some("https://api.minimax.chat/v1")), "minimax-cn");
        assert_eq!(hermes_env_provider("minimax", Some("https://api.minimaxi.com/anthropic")), "minimax-cn");
        assert_eq!(hermes_env_provider("minimax", Some("https://api.minimax.io/anthropic")), "minimax");
        assert_eq!(hermes_api_key_env_key("minimax-cn"), "MINIMAX_CN_API_KEY");
        assert_eq!(hermes_base_url_env_key("minimax-cn"), Some("MINIMAX_CN_BASE_URL"));
    }

    #[test]
    fn moonshot_uses_kimi_env_family() {
        assert_eq!(hermes_env_provider("moonshot", Some("https://api.moonshot.ai/v1")), "kimi-coding");
        assert_eq!(hermes_env_provider("moonshot", Some("https://api.moonshot.cn/v1")), "kimi-coding-cn");
        assert_eq!(hermes_api_key_env_key("kimi-coding"), "KIMI_API_KEY");
        assert_eq!(hermes_base_url_env_key("kimi-coding"), Some("KIMI_BASE_URL"));
    }

    #[test]
    fn infers_provider_from_known_base_url() {
        assert_eq!(hermes_provider_from_base_url("https://api.minimax.chat/v1"), Some("minimax"));
        assert_eq!(hermes_provider_from_base_url("https://api.minimaxi.com/anthropic"), Some("minimax"));
        assert_eq!(hermes_provider_from_base_url("https://api.minimax.io/anthropic"), Some("minimax"));
        assert_eq!(hermes_provider_from_base_url("https://api.anthropic.com/v1"), Some("anthropic"));
        assert_eq!(hermes_provider_from_base_url("https://openrouter.ai/api/v1"), Some("openrouter"));
    }

    #[test]
    fn provider_aliases_normalize_to_ui_keys() {
        assert_eq!(hermes_ui_provider("alibaba", None), "aliyun");
        assert_eq!(hermes_ui_provider("zai", None), "zhipu");
        assert_eq!(hermes_ui_provider("kimi-coding-cn", None), "moonshot");
        assert_eq!(hermes_ui_provider("minimax-cn", None), "minimax");
        assert_eq!(hermes_base_url_env_key("openrouter"), Some("OPENROUTER_BASE_URL"));
    }

    #[test]
    fn merge_env_file_clears_old_keys_when_switching_to_minimax_cn() {
        let existing = "\
...
";
        ...
        let new_pairs = vec![
            ("MINIMAX_CN_API_KEY".to_string(), "mini-key".to_string()),
            ("MINIMAX_CN_BASE_URL".to_string(), "https://api.minimax.chat/v1".to_string()),
        ];

        let merged = merge_env_file(existing, &managed_keys, &new_pairs);

        assert!(merged.contains("MINIMAX_CN_API_KEY=mini-key"));
        assert!(merged.contains("MINIMAX_CN_BASE_URL=https://api.minimax.chat/v1"));
        assert!(!merged.contains("ANTHROPIC_API_KEY="));
        assert!(!merged.contains("ANTHROPIC_BASE_URL="));
        assert!(merged.contains("KEEP_ME=1"));
    }

    #[test]
    fn merge_hermes_config_yaml_only_updates_top_level_model_block() {
        let existing = [
            "model:",
            "  default: old-model",
        ...
        let merged = merge_hermes_config_yaml(
            &existing,
        ...
        assert!(merged.contains("tts:\n  openai:"));
        assert!(merged.contains("  model:\n    provider: openrouter\n    model: anthropic/claude-sonnet-4"));
        assert_eq!(merged.lines().filter(|line| *line == "model:").count(), 1);
    }

    #[test]
    fn normalize_hermes_config_yaml_rewrites_bare_delegation_block() {
        let existing = [
            "model:",
            "  default: MiniMax-M2.7-highspeed",
            "terminal:",
        ...
        assert!(normalized.contains("delegation: {}"));
        assert!(normalized.contains("skills:\n  external_dirs: []"));
        assert!(!normalized.contains("delegation:\nskills:"));
    }
}
