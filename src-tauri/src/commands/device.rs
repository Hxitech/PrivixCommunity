...
/// 为授权 API 请求生成 Ed25519 签名
pub(crate) fn sign_license_payload(payload: &str) -> Result<(String, String, String), String> {
    let (device_id, pub_b64, signing_key) = get_or_create_key()?;
    let signature = signing_key.sign(payload.as_bytes());
    let sig_b64 = base64_url_encode(&signature.to_bytes());
    Ok((device_id, pub_b64, sig_b64))
}
...
pub fn create_connect_frame(nonce: String, gateway_token: String) -> Result<Value, String> {
    ...
    let frame = serde_json::json!({
        ...
        "params": {
            ...
            "locale": "zh-CN",
            "userAgent": format!("Privix/{}", env!("CARGO_PKG_VERSION")),
        }
    });
