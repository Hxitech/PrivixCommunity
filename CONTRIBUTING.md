...
> 📦 **仓库**: [github.com/Hxitech/ProspectClaw](https://github.com/Hxitech/ProspectClaw)
...
```bash
# 克隆仓库
git clone https://github.com/Hxitech/ProspectClaw.git
cd ProspectClaw
...
│   │   ├── agents.js           #   Agent 管理
│   │   ├── memory.js           #   记忆管理
│   │   ├── extensions.js       #   历史扩展工具页（当前未在产品导航中暴露）
│   │   ├── security.js         #   安全设置
│   │   ├── setup.js            #   初始设置向导
│   │   └── about.js            #   关于页面
│   ├── components/             # 通用组件
...
│   │   │   ├── pairing.rs      #     设备配对
│   │   │   ├── extensions.rs   #     历史扩展工具命令（cftunnel / clawapp，当前未在产品导航中暴露）
...
正式桌面发版默认同时输出三个 product profile，并统一使用下列对外包名：

| 内部 profile id | 对外包名 | 说明 |
|------|------|------|
| `invest_workbench` | `ProspectClaw Invest` | 投资工作台版 |
| `local_qa_kb` | `ProspectClaw Knowledge` | Agent 知识库版 |
| `doc_sop` | `ProspectClaw SOP` | Agent SOP 版 |
...
### `~/.openclaw/prospectclaw/<productProfileId>/clawpanel.json`
...
面向 macOS / Windows / Linux 桌面用户，从 [GitHub Releases](https://github.com/Hxitech/ProspectClaw/releases) 下载安装包。
...
```bash
curl -fsSL https://raw.githubusercontent.com/Hxitech/ProspectClaw/main/scripts/linux-deploy.sh | bash
```
