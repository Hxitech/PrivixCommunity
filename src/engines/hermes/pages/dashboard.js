...
const ICONS = {
  running: `<svg viewBox="0 0 24 24" fill="none" stroke="var(--success)" stroke-width="2.5" width="20" height="20"><circle cx="12" cy="12" r="10"/><polyline points="16 12 12 8 8 12"/><line x1="12" y1="16" x2="12" y2="8"/></svg>`,
  stopped: `<svg viewBox="0 0 24 24" fill="none" stroke="var(--error)" stroke-width="2.5" width="20" height="20"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>`,
...
export function render() {
  ...
  const CLI_COMMANDS = [
    { label: t('pages.engine.cliChat'),       desc: t('pages.engine.cliChatDesc'),      cmd: 'hermes chat' },
    { label: t('pages.engine.cliDoctor'),     desc: t('pages.engine.cliDoctorDesc'),    cmd: 'hermes doctor' },
    { label: t('pages.engine.cliVersion'),    desc: t('pages.engine.cliVersionDesc'),   cmd: 'hermes version' },
    { label: t('pages.engine.cliGwStart'),    desc: t('pages.engine.cliGwStartDesc'),   cmd: 'hermes gateway run' },
    { label: t('pages.engine.cliGwStop'),     desc: t('pages.engine.cliGwStopDesc'),    cmd: 'hermes gateway stop' },
    { label: t('pages.engine.cliUpgrade'),    desc: t('pages.engine.cliUpgradeDesc'),   cmd: 'uv tool install --reinstall "hermes-agent @ git+https://github.com/NousResearch/hermes-agent.git" --python 3.11' },
    { label: t('pages.engine.cliUninstall'),  desc: t('pages.engine.cliUninstallDesc'), cmd: 'uv tool uninstall hermes-agent' },
    { label: t('pages.engine.cliConfig'),     desc: t('pages.engine.cliConfigDesc'),    cmd: isWin ? `explorer ${configPath}` : `open ${configPath}` },
  ]
  ...
  function draw() {
    ...
    if (loading) {
      el.innerHTML = `
        <div class="page-header" style="display:flex;align-items:center;gap:12px">
          <h1 style="margin:0">${t('comp.header.page_dashboard')}</h1>
        </div>
    ...
    const displayModel = modelName || t('pages.engine.dashNoModel')
    ...
    const dropdownHtml = showDropdown && models.length
      ? `<div id="hm-model-dropdown" class="hermes-model-dropdown">${models.map(m =>
          `<div class="hermes-model-option${m === formModel ? ' is-active' : ''}" data-model="${esc(m)}">${esc(m)}</div>`
        ).join('')}</div>`
      : ''
    ...
    el.innerHTML = `
      ...
      <!-- 状态卡片行 -->
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:16px;margin-bottom:20px">
        <div class="card hermes-status-card ${gwRunning ? 'is-ok' : 'is-err'}">
          <div class="card-body" style="padding:16px">
            <div style="font-size:12px;color:var(--text-tertiary);margin-bottom:6px">${t('pages.engine.dashGatewayStatus')}</div>
            <div style="display:flex;align-items:center;gap:8px">
              ${gwRunning ? ICONS.running : ICONS.stopped}
              <span style="font-size:16px;font-weight:600">${gwRunning ? t('pages.engine.dashRunning') : t('pages.engine.dashStopped')}</span>
            </div>
          </div>
        </div>
        <div class="card">
          <div class="card-body" style="padding:16px">
            <div style="font-size:12px;color:var(--text-tertiary);margin-bottom:6px">${t('pages.engine.dashModel')}</div>
            <div style="font-size:14px;font-weight:600;word-break:break-all">${esc(displayModel)}</div>
          </div>
        </div>
        <div class="card">
          <div class="card-body" style="padding:16px">
            <div style="font-size:12px;color:var(--text-tertiary);margin-bottom:6px">${t('pages.engine.dashVersion')}</div>
            <div style="font-size:14px;font-weight:600">${version}</div>
          </div>
        </div>
        <div class="card">
          <div class="card-body" style="padding:16px">
            <div style="font-size:12px;color:var(--text-tertiary);margin-bottom:6px">${t('pages.engine.dashApiEndpoint')}</div>
      ...
      <div class="card" style="margin-bottom:20px">
        <div class="card-body" style="padding:0">
          <div class="hm-cfg-toggle" style="display:flex;align-items:center;justify-content:space-between;padding:14px 20px;cursor:pointer;user-select:none">
            <h3 style="margin:0;font-size:15px">${t('pages.engine.dashModelConfig')}</h3>
          ...
          <div style="${modelConfigCollapsed ? 'display:none' : 'padding:0 20px 20px'}">
            ...
            <div style="display:flex;gap:8px;align-items:flex-end;margin-bottom:12px">
              <label style="flex:1;display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--text-secondary)">
                ${t('pages.engine.configModel')}
                <div style="position:relative">
                  <input type="text" id="hm-cfg-model" class="input" value="${esc(formModel)}" placeholder="QC-B01" style="font-size:13px">
                  ${dropdownHtml}
                </div>
              </label>
              <button class="btn btn-sm btn-secondary hm-fetch-models" style="white-space:nowrap;flex-shrink:0" ${fetchBusy ? 'disabled' : ''}>${fetchBusy ? t('pages.engine.configFetching') : t('pages.engine.configFetchModels')}</button>
            </div>
            <div id="hm-cfg-msg" style="font-size:12px;min-height:16px;margin-bottom:8px">${cfgMsg}</div>
            <div style="display:flex;gap:8px">
              <button class="btn btn-primary btn-sm hm-save-model" ${modelBusy ? 'disabled' : ''}>${modelBusy ? '...' : t('pages.engine.configSaveBtn')}</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="card" style="margin-bottom:20px">
        <div class="card-body" style="padding:16px;display:flex;align-items:center;gap:12px;flex-wrap:wrap">
          ${!gwRunning ? `<button class="btn btn-primary btn-sm hm-dash-start" ${actionBusy ? 'disabled' : ''}>${actionBusy ? t('pages.engine.gatewayStarting') : t('pages.engine.dashStartGw')}</button>` : ''}
          ${gwRunning ? `<button class="btn btn-sm btn-secondary hm-dash-stop" ${actionBusy ? 'disabled' : ''}>${actionBusy ? t('pages.engine.dashStopping') : t('pages.engine.dashStopGw')}</button>` : ''}
          ${gwRunning ? `<button class="btn btn-sm btn-secondary hm-dash-restart" ${actionBusy ? 'disabled' : ''}>${actionBusy ? t('pages.engine.dashRestarting') : t('pages.engine.dashRestartGw')}</button>` : ''}
      ...
      <div class="card" style="margin-bottom:20px">
        <div class="card-body" style="padding:16px">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px">
            <h3 style="margin:0;font-size:15px">${t('pages.engine.dashConnectTarget')}</h3>
            <button class="btn btn-sm btn-secondary hm-detect-env" ${envDetecting ? 'disabled' : ''} style="font-size:11px;padding:2px 10px">${envDetecting ? t('pages.engine.dashDetecting') : t('pages.engine.dashDetectEnv')}</button>
          </div>
          <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:12px">
            <button class="btn btn-sm hm-connect-mode ${connectMode === 'local' ? 'btn-primary' : 'btn-secondary'}" data-mode="local" style="font-size:11px;padding:2px 10px">
              🖥️ ${t('pages.engine.dashConnLocal')}
            </button>
          ...
          <div style="display:flex;gap:8px;align-items:center">
            <button class="btn btn-sm btn-primary hm-apply-connect" style="font-size:11px;padding:2px 12px">${t('pages.engine.dashConnApply')}</button>
      ...
      <!-- 快捷操作 -->
      <div style="margin-bottom:12px;font-size:14px;font-weight:600">${t('pages.engine.dashQuickActions')}</div>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;margin-bottom:24px">
        <button class="card hm-dash-link" data-route="/h/chat" style="cursor:pointer;border:none;text-align:left">
          <div class="card-body" style="padding:16px;display:flex;align-items:center;gap:10px">
            ${ICONS.chat}
            <span style="font-size:14px;font-weight:500">${t('pages.engine.dashOpenChat')}</span>
          </div>
        </button>
        <button class="card hm-dash-link" data-route="/h/setup" style="cursor:pointer;border:none;text-align:left">
          <div class="card-body" style="padding:16px;display:flex;align-items:center;gap:10px">
            ${ICONS.config}
            <span style="font-size:14px;font-weight:500">${t('pages.engine.dashOpenSetup')}</span>
          </div>
        </button>
      </div>
      ...
      <div class="card" style="margin-bottom:20px">
        <div class="card-body" style="padding:20px">
          <h3 style="margin:0 0 4px;font-size:15px">${t('pages.engine.dashCliTitle')}</h3>
          <p style="margin:0 0 14px;font-size:12px;color:var(--text-tertiary)">${t('pages.engine.dashCliDesc')}</p>
  ...
  function bind() {
    ...
    el.querySelector('.hm-dash-start')?.addEventListener('click', async () => {
      ...
      showGwMsg(t('pages.engine.gatewayStarting'), false)
      try {
    ...
    el.querySelectorAll('.hermes-model-option').forEach(opt => {
  ...
  async function doFetchModels() {
    syncFormFromDom()
    if (!formBaseUrl) { cfgMsg = `<span style="color:var(--warning)">${t('pages.engine.configFetchNeedUrl')}</span>`; draw(); return }
    if (!formApiKey) { cfgMsg = `<span style="color:var(--warning)">${t('pages.engine.configFetchNeedKey')}</span>`; draw(); return }
    ...
    try {
      const fetchedModels = await api.hermesFetchModels({ baseUrl: formBaseUrl, apiKey: formApiKey, apiType })
      models = fetchedModels || []
      cfgMsg = `<span style="color:var(--success)">✓ ${t('pages.engine.configFetchSuccess', { count: models.length })}</span>`
  ...
  async function doSaveModel() {
    ...
    try {
      const result = await api.configureHermes(provider, formApiKey, formModel, formBaseUrl || null)
      cfgMsg = `<span style="color:var(--success)">✓ ${esc(result || '配置已保存')}</span>`
      // 刷新后端状态（不覆盖 form）
      try { hermesConfig = await api.hermesReadConfig() } catch (_) {}
      try { info = await api.checkHermes() } catch (_) {}
    } catch (e) {
