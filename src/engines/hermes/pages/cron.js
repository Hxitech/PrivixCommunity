...
function CRON_SHORTCUTS() {
  return [
    { expr: '*/5 * * * *', text: t('pages.engine.cronEvery5min') },
    { expr: '*/15 * * * *', text: t('pages.engine.cronEvery15min') },
    { expr: '0 * * * *', text: t('pages.engine.cronHourly') },
    { expr: '0 9 * * *', text: t('pages.engine.cronDaily9') },
    { expr: '0 18 * * *', text: t('pages.engine.cronDaily18') },
    { expr: '0 9 * * 1', text: t('pages.engine.cronMonday9') },
    { expr: '0 9 1 * *', text: t('pages.engine.cronMonthly1') },
  ]
}
...
function describeCron(raw) {
  ...
  if (min === '*' && hr === '*') return t('pages.engine.cronEveryMinute')
  if (min.startsWith('*/')) return t('pages.engine.cronEveryNMin').replace('{n}', min.slice(2))
  if (hr === '*' && min === '0') return t('pages.engine.cronHourlyOnTheHour')
  if (dow !== '*' && dom === '*') {
...
export function render() {
  ...
  async function gw(path, opts = {}) {
    return await api.hermesApiProxy({ method: (opts.method || 'GET').toUpperCase(), path, body: opts.body || null })
  }
  ...
  function draw() {
    ...
    el.innerHTML = `
      <div class="page-header" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
        <h1 style="margin:0">${t('comp.header.page_cron')}</h1>
        <div style="display:flex;gap:8px">
          <button class="btn btn-sm btn-secondary hm-cron-refresh" title="Refresh" style="padding:4px 10px">${ICONS.refresh}</button>
          <button class="btn btn-primary btn-sm hm-cron-create" ${!gwOnline ? 'disabled' : ''}>${t('pages.engine.cronCreate')}</button>
        </div>
      </div>
      ...
      ${!gwOnline ? `
        <div class="card"><div class="card-body" style="padding:32px;text-align:center;color:var(--text-tertiary)">
          ...
          ${t('pages.engine.chatGatewayOffline')}
        </div></div>
      ` : ''}
      ${gwOnline && !loading ? `
        <!-- 统计卡片 -->
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:20px">
          <div class="card"><div class="card-body" style="padding:12px 16px">
            <div style="font-size:11px;color:var(--text-tertiary);margin-bottom:4px">${t('pages.engine.cronTotal')}</div>
            <div style="font-size:20px;font-weight:700">${total}</div>
          </div></div>
          <div class="card"><div class="card-body" style="padding:12px 16px">
            <div style="font-size:11px;color:var(--text-tertiary);margin-bottom:4px">${t('pages.engine.cronRunning')}</div>
            <div style="font-size:20px;font-weight:700;color:var(--success)">${active}</div>
          </div></div>
          <div class="card"><div class="card-body" style="padding:12px 16px">
            <div style="font-size:11px;color:var(--text-tertiary);margin-bottom:4px">${t('pages.engine.cronPaused')}</div>
        ...
        ${total === 0 ? `
          <div class="card"><div class="card-body" style="padding:40px;text-align:center">
            ...
            <div style="font-size:15px;color:var(--text-secondary);margin-bottom:6px">${t('pages.engine.cronNoJobs')}</div>
            <div style="font-size:12px;color:var(--text-tertiary)">${t('pages.engine.cronNoJobsHint')}</div>
          </div></div>
        ` : renderJobList()}
      ` : ''}
      ${loading ? `
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap">
            <div style="flex:1;min-width:200px">
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">
                ...
                <span class="hermes-cron-badge ${j.paused ? 'is-paused' : 'is-active'}">${j.paused ? t('pages.engine.cronPaused') : t('pages.engine.cronActive')}</span>
              </div>
            ...
            <div style="display:flex;gap:6px;flex-shrink:0">
              ...
              <button class="btn btn-sm btn-secondary hm-cron-run" data-id="${id}" title="${t('pages.engine.cronRunNow')}" style="padding:5px 8px">${ICONS.zap}</button>
              <button class="btn btn-sm btn-secondary hm-cron-edit" data-id="${id}" title="${t('pages.engine.cronEdit')}" style="padding:5px 8px">${ICONS.edit}</button>
              <button class="btn btn-sm btn-secondary hm-cron-del" data-id="${id}" title="${t('pages.engine.cronDelete')}" style="padding:5px 8px;color:var(--error)">${ICONS.trash}</button>
            </div>
          </div>
        </div>
      </div>`
  ...
  function bindList() {
    ...
    el.querySelectorAll('.hm-cron-del').forEach(btn => {
      btn.addEventListener('click', async () => {
        const job = jobs.find(j => (j.id || j.name) === btn.dataset.id)
        const msg = t('pages.engine.cronConfirmDelete').replace('{name}', job?.name || btn.dataset.id)
  ...
  function drawForm() {
    ...
    el.innerHTML = `
      <div class="page-header" style="display:flex;align-items:center;gap:12px;margin-bottom:20px">
        ...
        <h1 style="margin:0">${isEdit ? t('pages.engine.cronEdit') + ' — ' + esc(editingJob.name) : t('pages.engine.cronCreate')}</h1>
      </div>
      ${errorMsg ? `<div style="color:var(--error);font-size:13px;margin-bottom:12px;padding:8px 12px;background:var(--error-muted, #fee2e2);border-radius:6px">${esc(errorMsg)}</div>` : ''}
      <div class="card">
        <div class="card-body" style="padding:24px;display:flex;flex-direction:column;gap:18px">
          ...
          <div>
            <label style="font-size:12px;font-weight:600;display:block;margin-bottom:6px">${t('pages.engine.cronName')}</label>
            <input class="input" id="hm-cron-name" value="${escAttr(editingJob.name)}" placeholder="${t('pages.engine.cronName')}" style="width:100%" ${isEdit ? 'disabled' : ''}>
          </div>

          <div>
            <label style="font-size:12px;font-weight:600;display:block;margin-bottom:6px">${t('pages.engine.cronSchedule')}</label>
          ...
          <div>
            <label style="font-size:12px;font-weight:600;display:block;margin-bottom:6px">${t('pages.engine.cronPrompt')}</label>
            <textarea class="input" id="hm-cron-prompt" rows="4" style="width:100%;resize:vertical;font-size:13px;line-height:1.5" placeholder="${t('pages.engine.cronPrompt')}">${esc(editingJob.prompt || '')}</textarea>
          </div>

          <div style="display:flex;gap:10px;margin-top:4px">
            <button class="btn btn-primary btn-sm hm-cron-save" ${busy ? 'disabled' : ''}>${busy ? t('pages.engine.cronSaving') : t('pages.engine.cronSave')}</button>
            <button class="btn btn-secondary btn-sm hm-cron-cancel">${t('pages.engine.cronCancel')}</button>
          </div>
        </div>
      </div>
    `
  ...
  function bindForm(isEdit, id) {
    ...
    el.querySelector('.hm-cron-save')?.addEventListener('click', async () => {
      ...
      if (!name) { errorMsg = t('pages.engine.cronNameRequired'); draw(); return }
      if (!schedule) { errorMsg = t('pages.engine.cronScheduleRequired'); draw(); return }
      if (!prompt) { errorMsg = t('pages.engine.cronPromptRequired'); draw(); return }
