  ...
  function draw() {
    ...
    el.innerHTML = `
      <div class="hm-skills-header">
        <span class="hm-skills-header-title">${t('comp.header.page_skills')}</span>
        <div class="hm-skills-header-right">
          <input type="text" id="hm-skills-search" class="hm-skills-header-search" placeholder="${t('pages.engine.skillsSearch')}" value="${escHtml(searchQuery)}">
          <span class="hm-skills-count">${totalSkillCount()} ${t('pages.engine.skillsTotal')}</span>
        </div>
      </div>
      <div class="hm-skills-layout">
        <div class="hm-skills-list-panel">
          <div class="hm-skills-list-scroll">
            ${loading ? `<div class="hm-skills-loading">${t('pages.engine.skillsLoading')}</div>` : ''}
            ${!loading && filtered.length === 0 ? `<div class="hm-skills-empty">${t('pages.engine.skillsEmpty')}</div>` : ''}
            ${!loading ? filtered.map(cat => `
              <div class="hm-skills-category">
                <div class="hm-skills-cat-header">
                  <span class="hm-skills-cat-name">${escHtml(cat.category === '_root' ? t('pages.engine.skillsUncategorized') : cat.category)}</span>
        ...
        <div class="hm-skills-detail-panel">
          ${!activeSkill ? `<div class="hm-skills-detail-empty">
            ...
            <span>${t('pages.engine.skillsSelectHint')}</span>
          </div>` : ''}
          ${activeSkill && loadingDetail ? `<div class="hm-skills-detail-loading">${t('pages.engine.skillsLoading')}</div>` : ''}
