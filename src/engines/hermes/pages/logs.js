...
export function render() {
  ...
  let searchQuery = ''
  let lineLimit = 200
  // 仅在加载完数据的那一帧自动滚到底部；键入搜索时不打断用户浏览位置
  ...
  async function loadEntries() {
    ...
    try {
      ...
      entries = [{ raw: `⚠️ ${t('pages.engine.logsLoadFailed')}: ${e.message || e}` }]
    }
    loading = false
  ...
  function draw() {
    ...
    el.innerHTML = `
      <div class="hm-logs-header">
        <span class="hm-logs-header-title">${t('comp.header.page_logs')}</span>
        <div class="hm-logs-header-actions">
          ...
          <select id="hm-logs-lines" class="hm-logs-select">
            ${[100, 200, 500, 1000].map(n => `<option value="${n}" ${n === lineLimit ? 'selected' : ''}>${n} ${t('pages.engine.logsLines')}</option>`).join('')}
          </select>
          <input type="text" id="hm-logs-search" class="hm-logs-search" placeholder="${t('pages.engine.logsSearch')}" value="${escHtml(searchQuery)}">
          <button class="btn btn-sm" id="hm-logs-refresh">${t('pages.engine.logsRefresh')}</button>
        </div>
      </div>
      <div class="hm-logs-layout">
        <div class="hm-logs-sidebar">
          <div class="hm-logs-sidebar-title">${t('pages.engine.logsFiles')}</div>
          ${logFiles.length === 0 ? `<div class="hm-logs-empty">${t('pages.engine.logsNoFiles')}</div>` : ''}
          ${logFiles.map(f => `
        ...
        <div class="hm-logs-main">
          <div class="hm-logs-toolbar">
            <div class="hm-logs-count">${filtered.length} ${t('pages.engine.logsEntries')}</div>
          </div>
          <div class="hm-logs-content" id="hm-logs-content">
            ${loading ? `<div class="hm-logs-loading">${t('pages.engine.logsLoading')}</div>` : ''}
            ${!loading && filtered.length === 0 ? `<div class="hm-logs-empty-content">${t('pages.engine.logsEmpty')}</div>` : ''}
  ...
  function renderEntry(e) {
    ...
    if (e.timestamp) {
      const m = e.timestamp.match(/\d{2}:\d{2}:\d{2}/)
      const time = m ? m[0] : e.timestamp
