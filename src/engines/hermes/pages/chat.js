...
function stripHiddenReasoning(text, final = false) {
  const src = String(text || '')
  const closedBlocks = src.replace(
    /<(?:think|thinking|reasoning|thought|REASONING_SCRATCHPAD)\b[^>]*>[\s\S]*?<\/(?:think|thinking|reasoning|thought|REASONING_SCRATCHPAD)>/gi,
    '',
  )
  const withoutOpenTail = closedBlocks.replace(
    /<(?:think|thinking|reasoning|thought|REASONING_SCRATCHPAD)\b[^>]*>[\s\S]*$/gi,
    '',
  )
  const withoutTags = withoutOpenTail.replace(
    /<\/?(?:think|thinking|reasoning|thought|REASONING_SCRATCHPAD)\b[^>]*>/gi,
    '',
  )
  return final ? withoutTags.trim() : withoutTags
}
...
function sessionTitle(s) {
  ...
  return first ? first.content.slice(0, 30) : t('pages.engine.chatNewSession')
}
...
export function render() {
  ...
  // 流式状态
  let pendingRawText = ''    // 原始 delta 文本（含隐藏推理）
  let pendingText = ''       // 过滤隐藏推理后的显示文本
  ...
  function updateStreamArea() {
    ...
    const textHtml = pendingText
      ...
      : (activeTools.length === 0 ? `<div class="hermes-chat-msg assistant"><div class="hermes-chat-bubble assistant"><span class="hermes-chat-typing">${t('pages.engine.chatThinking')}</span></div></div>` : '')
      ...
      <div class="hm-chat-layout">
        <div class="hm-chat-sidebar">
          <div class="hm-chat-sidebar-header">
            <span>${t('pages.engine.hermesChatTitle')}</span>
            <button class="hm-new-btn" title="${t('pages.engine.chatNewSession')}">
        ...
        <div class="hm-chat-main">
          <div class="hm-chat-model-bar">
            <span class="hm-model-label">${t('pages.engine.configModel')}:</span>
            ...
            <button class="hm-file-access-toggle ${fileAccessEnabled ? 'active' : ''}" id="hm-file-access-btn" title="${fileAccessEnabled ? t('pages.engine.fileAccessOn') : t('pages.engine.fileAccessOff')}">
              ...
              <span>${t('pages.engine.fileAccess')}</span>
            </button>
            <a href="#/h/dashboard" class="hm-model-link">${t('pages.engine.dashModelConfig')} →</a>
          </div>
          <div class="hermes-chat-messages" id="hm-chat-msgs">
            ${msgs.length === 0 ? `<div class="hermes-chat-empty">${t('pages.engine.chatEmptyHint')}</div>` : ''}
            ${msgs.map(m => renderMessage(m)).join('')}
          </div>
          <div class="hermes-chat-input-area">
            ${!gwOnline ? `<div class="hm-gw-offline">${t('pages.engine.chatGatewayOffline')}</div>` : ''}
            <div style="position:relative">
              ${showSlash ? renderSlashMenu() : ''}
              <div class="hm-chat-input-wrap">
                <textarea id="hm-chat-input" rows="1" placeholder="${t('pages.engine.chatPlaceholder')}" ${!gwOnline ? 'disabled' : ''}></textarea>
                <button class="btn btn-primary hm-chat-send" ${!gwOnline || streaming ? 'disabled' : ''}>${streaming ? '...' : t('pages.engine.chatSend')}</button>
              </div>
            </div>
          </div>
        </div>
  ...
  async function setupRunListeners() {
    ...
    const u1 = await tauriListen('hermes-run-delta', (e) => {
      pendingRawText += e.payload?.delta || ''
      pendingText = stripHiddenReasoning(pendingRawText)
      updateStreamArea()
    })
    ...
    const u3 = await tauriListen('hermes-run-done', (e) => {
      const cur = active()
      if (!cur) return
      const rawOutput = e.payload?.output || pendingRawText || pendingText || ''
      const output = stripHiddenReasoning(rawOutput, true) || '(empty)'
      // 存储工具摘要（含输入输出详情）
  ...
  async function sendMessage() {
    ...
    try {
      ...
      await api.hermesAgentRun({ input: text, sessionId: cur.id, conversationHistory: history.length ? history : null, instructions })
      // run 完成后事件监听会处理结果
    } catch (e) {
      const msg = String(e.message || e).replace(/^Error:\s*/, '')
      cur.messages.push({ role: 'assistant', content: `⚠️ ${t('pages.engine.chatError', { error: msg })}` })
      streaming = false
      pendingRawText = ''
      pendingText = ''
      activeTools = []
