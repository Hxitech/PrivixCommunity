...
| \`openclaw gateway --container\` | 以容器模式启动（3.24+，优化进程管理和信号处理） |
...
- \`OPENCLAW_CONTAINER=true\` — 容器模式（3.24+），等同于 \`--container\` 标志
...
### 阿里云 DashScope（3.23+）
- 中国区端点：\`https://dashscope.aliyuncs.com/compatible-mode/v1\`
- 国际区端点：\`https://dashscope-intl.aliyuncs.com/compatible-mode/v1\`（按量付费）
- 支持 Qwen Max/Plus/Turbo/VL Max、Qwen 2.5 Max 等模型
...
- 环境变量或配置中设置
- 支持 guild 级别配置
- 自动讨论串（3.24+）：开启 \`channels.discord.autoThread\` 后，每条消息自动创建 Thread，标题由 LLM 生成
