...
import { registerRoute, setDefaultRoute, withTimeout } from '../router.js'

/** 已注册引擎 ID 常量;避免散落在消费方的 stringly-typed 字符串 */
...
export async function activateEngine(id, persist = true) {
  ...
  // 清理旧引擎 + 重置 API 缓存与 in-flight,避免旧引擎 pending 请求阻塞新引擎页面(上游 v0.13.3 修复)
  ...
  // 10s 超时:避免引擎 boot 慢请求把 sidebar 卡在 loading
  if (persist && engine.boot) {
    try {
      await withTimeout(engine.boot(), 10000, 'engine boot timeout')
    } catch (e) {
