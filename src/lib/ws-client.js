...
export class WsClient {
  constructor() {
    ...
    this._preferredSessionKey = null
  ...
  /**
   * 设置确定性 session key（客户端生成，重连时复用同一 session）
   * 需在 connect() 前调用
   */
  setPreferredSessionKey(key) {
    this._preferredSessionKey = key || null
  }
  ...
  async _handleConnectSuccess(payload) {
    this._autoPairAttempts = 0
    this._authRetryCount = 0
    ...
    // 确定性 session key：重连时尝试恢复旧 session（await 确保 key 确定后再通知消费者）
    if (this._preferredSessionKey && this._preferredSessionKey !== this._sessionKey) {
      try {
        await this.request('sessions.resume', { key: this._preferredSessionKey })
        console.log('[ws] 成功恢复 preferred session:', this._preferredSessionKey)
        this._sessionKey = this._preferredSessionKey
      } catch {
        console.log('[ws] sessions.resume 不可用，使用 Gateway 分配的 key')
      }
    }
