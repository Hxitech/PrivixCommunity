...
function _setGatewayRunning(val, foreign = false) {
  ...
  if (changed) {
    ...
    } else if (!isTauri && wasRunning && !_userStopped && !_isUpgrading && _openclawReady && !foreign) {
      _gatewayRunningSince = 0
      // Gateway 意外停止，尝试自动重启
      _tryAutoRestart()
    } else if (!val) {
...
export async function refreshGatewayStatus() {
  try {
    ...
    if (services?.length > 0) {
      const gw = services.find?.(s => s.label === 'ai.openclaw.gateway') || services[0]
      const nowRunning = gw?.running === true
      const foreignRunning = nowRunning && gw?.owned_by_current_instance === false
      if (nowRunning && !foreignRunning) {
        _gwStopCount = 0
        if (!_gatewayRunning) {
