...
function getCategories() {
  return [
    { key: 'memory', label: t('pages.memory.category_memory_label'), desc: t('pages.memory.category_memory_desc') },
    { key: 'archive', label: t('pages.memory.category_archive_label'), desc: t('pages.memory.category_archive_desc') },
    { key: 'core', label: t('pages.memory.category_core_label'), desc: t('pages.memory.category_core_desc') },
  ]
}
...
export async function render() {
  ...
  page.innerHTML = `
    <div class="page-header">
      <h1 class="apple-section">${t('pages.memory.title')}</h1>
      <div class="page-actions" style="display:flex;align-items:center;gap:var(--space-sm)">
        <label style="font-size:var(--font-size-sm);color:var(--text-tertiary)">${t('pages.memory.agent_label')}</label>
    ...
    <div class="memory-layout">
      <div class="memory-sidebar">
        <div style="padding:0 var(--space-sm) var(--space-sm);display:flex;gap:4px">
          <button class="btn btn-sm btn-secondary" id="btn-new-file" style="flex:1">${t('pages.memory.btn_new_file')}</button>
          <button class="btn btn-sm btn-danger" id="btn-del-file" disabled style="flex:1">${t('pages.memory.btn_delete_file')}</button>
        </div>
        <div style="padding:0 var(--space-sm) var(--space-sm)">
          <button class="btn btn-sm btn-secondary" id="btn-export-zip" style="width:100%">${t('pages.memory.btn_export_zip')}</button>
        </div>
      ...
      <div class="memory-editor">
        <div class="editor-toolbar">
          <span id="current-file" style="font-size:var(--font-size-sm);color:var(--text-tertiary)">${t('pages.memory.file_placeholder')}</span>
          <div style="display:flex;gap:8px">
            <button class="btn btn-sm btn-secondary" id="btn-download" disabled>${t('pages.memory.btn_download')}</button>
            <button class="btn btn-sm btn-secondary" id="btn-preview" disabled>${t('pages.memory.btn_preview')}</button>
            <button class="btn btn-sm btn-primary" id="btn-save-file" disabled>${t('pages.memory.btn_save_file')}</button>
          </div>
        </div>
        <textarea class="editor-area" id="file-editor" placeholder="${t('pages.memory.editor_placeholder')}" disabled></textarea>
      </div>
    </div>
  `
  ...
  page.querySelector('#btn-new-file').onclick = () => {
    showModal({
      title: t('pages.memory.new_file_title'),
      fields: [{ name: 'filename', label: t('pages.memory.new_file_name_label'), placeholder: t('pages.memory.new_file_name_placeholder'), hint: '建议使用 .md 格式，文件将保存到当前分类目录下' }],
      onConfirm: async ({ filename }) => {
        ...
        try {
          ...
          toast(t('pages.memory.save_failed', { error: String(e) }), 'error')
        }
      },
    })
  }
  ...
  page.querySelector('#btn-del-file').onclick = async () => {
    ...
    const yes = await showConfirm(t('pages.memory.delete_confirm', { name }))
    if (!yes) return
    try {
      await api.deleteMemoryFile(state.currentPath, state.agentId)
      toast(t('pages.memory.delete_success'), 'success')
      state.currentPath = null
      resetEditor(page)
      loadFiles(page, state)
    } catch (e) {
      toast(t('pages.memory.delete_failed', { error: String(e) }), 'error')
    }
  }
...
async function loadFiles(page, state) {
  ...
  try {
    ...
    if (!files || !files.length) {
      tree.innerHTML = '<div style="color:var(--text-tertiary);padding:12px">' + t('pages.memory.empty_title') + '</div>'
      return
    }
    renderFileTree(page, state, files)
  } catch (e) {
    tree.innerHTML = '<div style="color:var(--error);padding:12px">' + t('pages.memory.load_failed', { error: String(e) }) + '</div>'
    toast(t('pages.memory.load_failed', { error: String(e) }), 'error')
  }
}
...
async function loadFileContent(page, state) {
  ...
  btnPreview.textContent = t('pages.memory.btn_preview')
  ...
  try {
    ...
    editor.value = t('pages.memory.load_failed', { error: String(e) })
    toast(t('pages.memory.load_failed', { error: String(e) }), 'error')
  }
}
...
function resetEditor(page) {
  ...
  const previewEl = page.querySelector('#md-preview')
  if (previewEl) previewEl.remove()
  page.querySelector('#current-file').textContent = t('pages.memory.file_placeholder')
  page.querySelector('#btn-save-file').disabled = true
  page.querySelector('#btn-preview').disabled = true
  page.querySelector('#btn-preview').textContent = t('pages.memory.btn_preview')
...
async function saveFile(page, state) {
  ...
  try {
    ...
    toast(t('pages.memory.save_success'), 'success')
  } catch (e) {
    toast(t('pages.memory.save_failed', { error: String(e) }), 'error')
  }
}
...
function togglePreview(page) {
  ...
  if (previewEl) {
    ...
    previewEl.remove()
    editor.style.display = ''
    btn.textContent = t('pages.memory.btn_preview')
  } else {
    // 进入预览
...
async function downloadCurrentFile(page, state) {
  ...
  try {
    ...
    toast(t('pages.memory.export_failed', { error: String(e) }), 'error')
  }
}
...
async function exportZip(state) {
  try {
    ...
    try {
      ...
      await open(dir)
      toast(t('pages.memory.export_success') + `: ${label} → ${zipPath}`, 'success')
    } catch {
      // fallback：仅显示路径
      toast(t('pages.memory.export_success') + `: ${label} → ${zipPath}`, 'success')
    }
