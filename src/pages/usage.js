...
export async function render() {
  ...
  page.innerHTML = `
    <div class="page-header">
      <h1 class="apple-section">${t('pages.usage.title')}</h1>
      <p class="apple-body-secondary">${t('pages.usage.desc')}</p>
    </div>
    <div class="usage-toolbar" style="display:flex;gap:8px;align-items:center;margin-bottom:var(--space-lg);flex-wrap:wrap">
      <button class="btn btn-sm ${_days === 1 ? 'btn-primary' : 'btn-secondary'}" data-days="1">${t('pages.usage.btn_today')}</button>
      <button class="btn btn-sm ${_days === 7 ? 'btn-primary' : 'btn-secondary'}" data-days="7">${t('pages.usage.btn_7days')}</button>
      <button class="btn btn-sm ${_days === 30 ? 'btn-primary' : 'btn-secondary'}" data-days="30">${t('pages.usage.btn_30days')}</button>
      <button class="btn btn-sm btn-pill-filled" id="btn-usage-refresh">${icon('refresh-cw', 14)} ${t('pages.usage.btn_refresh')}</button>
    </div>
...
async function loadUsage(page) {
  ...
  try {
    ...
    el.innerHTML = `<div class="usage-empty">
      <div style="color:var(--error);margin-bottom:8px">${t('pages.usage.load_fail', { error: String(e?.message || e) })}</div>
      <div class="form-hint">${t('pages.usage.load_fail_hint')}</div>
      <button class="btn btn-secondary btn-sm" style="margin-top:8px" onclick="this.closest('.page').querySelector('#btn-usage-refresh').click()">${t('pages.usage.btn_retry')}</button>
    </div>`
  }
}
...
function renderUsage(el, data) {
  ...
  const overviewHtml = `
    <div class="stat-cards" style="margin-bottom:var(--space-lg)">
      <div class="stat-card">
        <div class="stat-card-header"><span class="stat-card-label">${t('pages.usage.card_messages')}</span></div>
      ...
      <div class="stat-card">
        <div class="stat-card-header"><span class="stat-card-label">${t('pages.usage.card_tool_calls')}</span></div>
      ...
      <div class="stat-card">
        <div class="stat-card-header"><span class="stat-card-label">${t('pages.usage.card_errors')}</span></div>
      ...
      <div class="stat-card">
        <div class="stat-card-header"><span class="stat-card-label">${t('pages.usage.card_tokens')}</span></div>
        <div class="stat-card-value">${fmtTokens(totals.totalTokens)}</div>
        <div class="stat-card-meta">${t('pages.usage.meta_input', { n: fmtTokens(totals.input) })} · ${t('pages.usage.meta_output', { n: fmtTokens(totals.output) })}</div>
      </div>
      <div class="stat-card">
        <div class="stat-card-header"><span class="stat-card-label">${t('pages.usage.card_cost')}</span></div>
        <div class="stat-card-value">${fmtCost(totals.totalCost)}</div>
        <div class="stat-card-meta">${t('pages.usage.meta_input_cost', { n: fmtCost(totals.inputCost) })} · ${t('pages.usage.meta_output_cost', { n: fmtCost(totals.outputCost) })}</div>
      </div>
      <div class="stat-card">
        <div class="stat-card-header"><span class="stat-card-label">${t('pages.usage.card_sessions')}</span></div>
  ...
  const unknownLabel = t('pages.usage.unknown')
  const topModels = renderTop(t('pages.usage.top_models'),
    a.byModel, m => m.model || unknownLabel, m => fmtCost(m.totals?.totalCost) + ' · ' + fmtTokens(m.totals?.totalTokens))
  const topProviders = renderTop(t('pages.usage.top_providers'),
    a.byProvider, p => p.provider || unknownLabel, p => fmtCost(p.totals?.totalCost) + ' · ' + t('pages.usage.calls_count', { n: p.count }))
  const topTools = renderTop(t('pages.usage.top_tools'),
    (tools.tools || []), item => item.name, item => t('pages.usage.calls_suffix', { n: item.count }))
  const topAgents = renderTop(t('pages.usage.top_agents'),
    a.byAgent, ag => ag.agentId || 'main', ag => fmtCost(ag.totals?.totalCost))
  const topChannels = renderTop(t('pages.usage.top_channels'),
  ...
  const tokenBreakdownHtml = `
    <div class="config-section" style="margin-top:var(--space-lg)">
      <div class="config-section-title">${t('pages.usage.section_token_breakdown')}</div>
      <div style="display:flex;gap:var(--space-lg);flex-wrap:wrap;padding:var(--space-md)">
        <div><span style="display:inline-block;width:10px;height:10px;background:var(--error);border-radius:2px;margin-right:6px"></span>${t('pages.usage.token_output', { n: fmtTokens(totals.output) })}</div>
        <div><span style="display:inline-block;width:10px;height:10px;background:var(--accent);border-radius:2px;margin-right:6px"></span>${t('pages.usage.token_input', { n: fmtTokens(totals.input) })}</div>
        <div><span style="display:inline-block;width:10px;height:10px;background:var(--success);border-radius:2px;margin-right:6px"></span>${t('pages.usage.token_cache_read', { n: fmtTokens(totals.cacheRead) })}</div>
        <div><span style="display:inline-block;width:10px;height:10px;background:var(--warning);border-radius:2px;margin-right:6px"></span>${t('pages.usage.token_cache_write', { n: fmtTokens(totals.cacheWrite) })}</div>
      </div>
    </div>
  `
  ...
  if (daily.length > 0) {
    ...
    dailyHtml = `
      <div class="config-section" style="margin-top:var(--space-lg)">
        <div class="config-section-title">${t('pages.usage.section_daily')}</div>
