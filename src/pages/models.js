...
export async function render() {
  ...
  page.innerHTML = `
    <div class="page-header">
      <h1 class="apple-section">${t('pages.models.title')}</h1>
      <p class="apple-body-secondary">${t('pages.models.page_desc')}</p>
    </div>
    <div class="config-actions">
      <button class="btn btn-pill-filled" id="btn-add-provider">${t('pages.models.btn_add_provider')}</button>
      <button class="btn btn-secondary btn-sm" id="btn-undo" disabled>${t('pages.models.btn_undo')}</button>
    </div>
    <div class="form-hint" style="margin-bottom:var(--space-md)">
      ${t('pages.models.provider_hint')}
    </div>
    <div id="default-model-bar"></div>
    <div style="margin-bottom:var(--space-md)">
      <input class="form-input" id="model-search" placeholder="${t('pages.models.search_placeholder')}" style="max-width:360px">
    </div>
...
async function loadConfig(page, state) {
  ...
  try {
    ...
    if (before !== after) {
      ...
      toast(t('pages.models.toast_url_fixed'), 'info')
    }
    renderDefaultBar(page, state)
    renderProviders(page, state)
  } catch (e) {
    listEl.innerHTML = `<div style="color:var(--error);padding:20px">${t('pages.models.toast_load_failed', { error: e })}</div>`
    toast(t('pages.models.toast_load_failed', { error: e }), 'error')
  }
}
...
function getApiTypeLabel(apiType) {
  return API_TYPES.find(at => at.value === apiType)?.label || apiType || t('pages.models.api_type_unknown')
}
...
function renderDefaultBar(page, state) {
  ...
  bar.innerHTML = `
    <div class="config-section" style="margin-bottom:var(--space-lg)">
      <div class="config-section-title">${t('pages.models.section_current_config')}</div>
      <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
        <div>
          <span style="font-size:var(--font-size-sm);color:var(--text-tertiary)">${t('pages.models.label_primary_model')}</span>
          <span style="font-family:var(--font-mono);font-size:var(--font-size-sm);color:${primary ? 'var(--success)' : 'var(--error)'}">${primary || t('pages.models.primary_unset')}</span>
        </div>
        <div>
          <span style="font-size:var(--font-size-sm);color:var(--text-tertiary)">${t('pages.models.label_fallback_models')}</span>
          <span style="font-size:var(--font-size-sm);color:var(--text-secondary)">${fallbacks.length ? fallbacks.join(', ') : t('pages.models.fallback_none')}</span>
        </div>
      </div>
      <div class="form-hint" style="margin-top:6px">${t('pages.models.fallback_hint')}</div>
    </div>
  `
}
  ...
  if (!keys.length) {
    listEl.innerHTML = `
      <div style="color:var(--text-tertiary);padding:20px;text-align:center">
        ${t('pages.models.empty_providers')}
      </div>`
    return
  }
    ...
    return `
      <div class="config-section" data-provider="${key}">
        <div class="config-section-title" style="display:flex;justify-content:space-between;align-items:center">
          <span>${key} <span style="font-size:var(--font-size-xs);color:var(--text-tertiary);font-weight:400">${getApiTypeLabel(p.api)} · ${t('pages.models.label_models_count', { count: models.length })}</span></span>
          <div style="display:flex;gap:8px">
            <button class="btn btn-sm btn-secondary" data-action="edit-provider">${t('pages.models.btn_edit_provider')}</button>
            <button class="btn btn-sm btn-secondary" data-action="add-model">${t('pages.models.btn_add_model')}</button>
            <button class="btn btn-sm btn-secondary" data-action="fetch-models">${t('pages.models.btn_fetch_models')}</button>
            <button class="btn btn-sm btn-danger" data-action="delete-provider">${t('pages.models.btn_delete_provider')}</button>
          </div>
        </div>
        ${models.length >= 2 ? `
        <div style="display:flex;gap:6px;margin-bottom:var(--space-sm);align-items:center">
          <button class="btn btn-sm btn-secondary" data-action="batch-test">${t('pages.models.btn_batch_test')}</button>
          <button class="btn btn-sm btn-secondary" data-action="select-all">${t('pages.models.btn_select_all')}</button>
          <button class="btn btn-sm btn-danger" data-action="batch-delete">${t('pages.models.btn_batch_delete')}</button>
        ...
        <div class="provider-models">
          ...
          ${hiddenCount > 0 ? `<div style="font-size:var(--font-size-xs);color:var(--text-tertiary);padding:4px 0">${t('pages.models.hidden_count', { count: hiddenCount })}</div>` : ''}
        </div>
      </div>
    `
  }).join('')
...
function renderModelCards(providerKey, models, primary, search) {
  if (!models.length) {
    return `<div style="color:var(--text-tertiary);font-size:var(--font-size-sm);padding:8px 0">${t('pages.models.empty_models')}</div>`
  }
  return models.map((m) => {
    const id = typeof m === 'string' ? m : m.id
    ...
    if (m.contextWindow) meta.push(t('pages.models.context_k', { size: m.contextWindow / 1000 }))
    // 测试状态标签：成功显示耗时，失败显示不可用
    let latencyTag = ''
    if (m.testStatus === 'fail') {
      latencyTag = `<span style="font-size:var(--font-size-xs);padding:1px 6px;border-radius:var(--radius-sm);background:var(--error-muted, #fee2e2);color:var(--error)" title="${(m.testError || '').replace(/"/g, '&quot;')}">${t('pages.models.badge_unavailable')}</span>`
      ...
      <div class="model-card" data-model-id="${id}" data-full="${full}"
        ...
        <div style="flex:1;min-width:0">
          <div style="display:flex;align-items:center;gap:8px">
            ...
            ${isPrimary ? `<span style="font-size:var(--font-size-xs);background:var(--success);color:var(--text-inverse);padding:1px 6px;border-radius:var(--radius-sm)">${t('pages.models.badge_primary')}</span>` : ''}
            ${m.reasoning ? `<span style="font-size:var(--font-size-xs);background:var(--accent-muted);color:var(--accent);padding:1px 6px;border-radius:var(--radius-sm)">${t('pages.models.badge_reasoning')}</span>` : ''}
            ${latencyTag}
          </div>
          <div style="font-size:var(--font-size-xs);color:var(--text-tertiary);margin-top:2px">${meta.join(' · ') || ''}</div>
        </div>
        <div style="display:flex;gap:6px;flex-shrink:0">
          <button class="btn btn-sm btn-secondary" data-action="test-model">${t('pages.models.btn_test')}</button>
          ${!isPrimary ? `<button class="btn btn-sm btn-secondary" data-action="set-primary">${t('pages.models.btn_set_primary')}</button>` : ''}
          <button class="btn btn-sm btn-secondary" data-action="edit-model">${t('pages.models.btn_edit_model')}</button>
          <button class="btn btn-sm btn-danger" data-action="delete-model">${t('pages.models.btn_delete_model')}</button>
        </div>
      </div>
    `
  }).join('')
...
function formatTestTime(ts) {
  ...
  if (diff < 60000) return t('pages.models.test_just_now')
  if (diff < 3600000) return t('pages.models.test_minutes_ago', { count: Math.floor(diff / 60000) })
  if (diff < 86400000) return t('pages.models.test_hours_ago', { count: Math.floor(diff / 3600000) })
  return t('pages.models.test_days_ago', { count: Math.floor(diff / 86400000) })
}
...
async function undo(page, state) {
  ...
  toast(t('pages.models.toast_undone'), 'info')
}
...
function normalizeProviderUrls(config) {
  ...
  for (const [, p] of Object.entries(providers)) {
    ...
    } else if (apiType !== 'google-gemini' && apiType !== 'ollama') {
...
function applyDefaultModel(state) {
  ...
  const needFallbacks = !defaults.model.fallbacks || defaults.model.fallbacks.length === 0
  const needModelsMap = !defaults.models || Object.keys(defaults.models).length === 0
  if (needFallbacks || needModelsMap) {
    const allModels = collectAllModels(state.config)
    if (needFallbacks) {
  ...
  // 不再强制覆盖各 agent 的 model.primary (#142)
  // 子 Agent 模型配置是独立的，用户可以为不同 Agent 设置不同模型
}
...
function addProvider(page, state) {
  ...
  overlay.innerHTML = `
    <div class="modal" style="max-height:85vh;overflow-y:auto">
      ...
      <div class="form-group">
        ...
        <div class="form-hint">先选服务商；若该服务商有多种接入方式，会继续显示二级选项。默认只需填写 API Key。</div>
      ...
      <div class="form-group">
        ...
        <div class="form-hint" id="provider-key-hint">自定义标识名，用于区分不同来源</div>
      </div>
      <div class="form-group">
  ...
  const apiKeyInput = overlay.querySelector('[data-name="apiKey"]')
  const detailEl = overlay.querySelector('#preset-detail')
  const keyHintEl = overlay.querySelector('#provider-key-hint')
  let selectedChoice = null

  function applyPresetChoice(choice) {
    selectedChoice = choice
    keyInput.value = choice.providerKey || ''
    baseUrlInput.value = choice.baseUrl || ''
    apiSelect.value = choice.api || 'openai-completions'
    if (choice.lockProviderKey) {
      keyInput.readOnly = true
      keyHintEl.innerHTML = `按 OpenClaw 原生 provider key 固定为 <code>${choice.providerKey}</code>`
    } else {
      keyInput.readOnly = false
      keyHintEl.textContent = '自定义标识名，用于区分不同来源'
    }
    if (!apiKeyInput.value) apiKeyInput.focus()
  }

  function renderPresetDetail(preset, activeChoiceKey = null) {
    const activeChoice = resolveProviderPresetChoice(preset.key, activeChoiceKey)
    if (!activeChoice) {
      detailEl.style.display = 'none'
      return
    }

    applyPresetChoice(activeChoice)
    const choiceButtons = Array.isArray(preset.choices) && preset.choices.length > 1
      ? `
        <div style="margin-top:10px">
          <div style="font-size:12px;font-weight:600;margin-bottom:6px">接入方式</div>
          <div style="display:flex;flex-wrap:wrap;gap:6px">
            ${preset.choices.map(choice => `
              <button class="btn btn-sm btn-secondary preset-choice-btn" data-choice="${choice.choiceKey}" style="${choice.choiceKey === activeChoice.choiceKey ? 'opacity:1;border-color:var(--accent);color:var(--accent)' : 'opacity:0.75'}">${choice.label}</button>
            `).join('')}
          </div>
        </div>`
      : ''
    ...
    const defaultModelLine = activeChoice.defaultModel
      ? `<div class="form-hint" style="margin-top:8px">会自动补充默认模型 <code>${activeChoice.providerKey}/${activeChoice.defaultModel}</code>，但不会自动切成当前主模型。</div>`
      : ''
    const noteLine = activeChoice.note
      ? `<div class="form-hint" style="margin-top:6px">${activeChoice.note}</div>`
      : ''
    const linkLine = activeChoice.signupUrl || preset.site
      ? `<a href="${activeChoice.signupUrl || preset.site}" target="_blank" style="color:var(--accent);text-decoration:none;font-size:12px;margin-top:6px;display:inline-block">→ 打开接入说明 / 官网</a>`
      : ''

    detailEl.innerHTML = `
      ${preset.hint ? `<div style="font-weight:600;margin-bottom:4px">${preset.hint}</div>` : ''}
      ${preset.desc ? `<div style="color:var(--text-secondary);line-height:1.6">${preset.desc}</div>` : ''}
      ${choiceButtons}
      ${defaultModelLine}
      ${noteLine}
      ${linkLine}
    ...
    detailEl.querySelectorAll('.preset-choice-btn').forEach(btn => {
      btn.onclick = () => renderPresetDetail(preset, btn.dataset.choice)
    })
  }
  ...
  overlay.querySelector('[data-action="confirm"]').onclick = () => {
    ...
    let message = `已添加服务商: ${key}`
    if (selectedChoice) {
      const result = upsertProviderWithDefaults(state.config, selectedChoice, {
        providerKey: selectedChoice.lockProviderKey ? selectedChoice.providerKey : key,
        baseUrl: baseUrl || '',
        apiKey: apiKey || '',
        api: apiType,
      })
      message = result.created
        ? `已添加服务商: ${result.providerKey}`
        : `已更新服务商: ${result.providerKey}`
      if (result.addedModelIds.length > 0) {
        message += `，并补充 ${result.addedModelIds.length} 个默认模型`
      }
    } else {
