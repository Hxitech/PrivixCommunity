...
export async function render() {
  ...
  page.innerHTML = `
    <div class="page-header">
      <h1 class="page-title apple-section">${t('pages.route_map.title')}</h1>
      <div class="page-actions" style="display:flex;align-items:center;gap:var(--space-sm)">
        <span class="apple-micro">${t('pages.route_map.clickToNavigate')}</span>
        <button class="btn btn-pill-filled" id="rm-refresh">${t('pages.route_map.refresh')}</button>
      </div>
    </div>
    <p class="apple-body-secondary" style="margin-bottom:var(--space-md)">${t('pages.route_map.subtitle')}</p>
  ...
  async function loadAndRender() {
    ...
    canvas.innerHTML = '<div class="stat-card loading-placeholder" style="height:300px;display:flex;align-items:center;justify-content:center;color:var(--text-tertiary)">' + t('pages.route_map.loading') + '</div>'
    ...
    try {
      ...
      if (agents.length === 0 && platforms.length === 0) {
        canvas.innerHTML = `<div class="stat-card" style="padding:var(--space-xl);text-align:center">
          <div class="apple-tile">${t('pages.route_map.noData')}</div>
        </div>`
        statsEl.innerHTML = ''
        return
      }
      ...
      statsEl.innerHTML = `
        <div class="route-map-stat"><span class="route-map-stat-num apple-tile">${agents.length}</span><span class="route-map-stat-label apple-caption">${t('pages.route_map.statsAgents')}</span></div>
        <div class="route-map-stat"><span class="route-map-stat-num apple-tile">${platforms.length}</span><span class="route-map-stat-label apple-caption">${t('pages.route_map.statsChannels')}</span></div>
        <div class="route-map-stat"><span class="route-map-stat-num apple-tile">${bindings.length}</span><span class="route-map-stat-label apple-caption">${t('pages.route_map.statsBindings')}</span></div>
        ${subAgentCount ? `<div class="route-map-stat"><span class="route-map-stat-num apple-tile">${subAgentCount}</span><span class="route-map-stat-label apple-caption">${t('pages.route_map.subAgentRelations')}</span></div>` : ''}
      `
...
function renderTopology(container, agents, bindings, platforms) {
  ...
  const agentNodes = agents.map((a, i) => {
    ...
    const emoji = identity.emoji || '◆'  // 几何菱形替代 🤖(用户自定义 emoji 保留)
  ...
  for (const b of bindings) {
    ...
    let hint = ''
    if (peer) hint = t('pages.route_map.peer')
    else if (accountId) hint = `${t('pages.route_map.account')}: ${accountId}`
  ...
  for (const ch of channelNodes) {
    if (!boundChannels.has(ch.id) && ch.enabled) {
      edges.push({ from: ch.id, to: defaultAgentId, enabled: true, hint: t('pages.route_map.defaultAgent'), channel: ch.id, agentId: defaultAgentId, implicit: true })
    }
  }
  ...
  // Column headers
  svg += `<text x="${PAD_LEFT + NODE_W / 2}" y="30" text-anchor="middle" class="route-map-col-label">${t('pages.route_map.channels')}</text>`
  svg += `<text x="${PAD_LEFT + NODE_W + COL_GAP + NODE_W / 2}" y="30" text-anchor="middle" class="route-map-col-label">${t('pages.route_map.agents')}</text>`
  svg += `<text x="${PAD_LEFT + NODE_W + COL_GAP / 2}" y="30" text-anchor="middle" class="route-map-col-label" style="font-size:11px;opacity:0.5">${t('pages.route_map.bindings')}</text>`
  ...
  for (const n of channelNodes) {
    ...
    svg += `<g class="route-map-node" data-nav="channels" style="cursor:pointer;opacity:${opacity}">
      ...
      <text x="${n.x + 40}" y="${n.y + NODE_H / 2 + 12}" class="route-map-node-sub">${n.enabled ? t('pages.route_map.enabled') : t('pages.route_map.disabled')}</text>
    </g>`
  }
  ...
  for (const n of agentNodes) {
    svg += `<g class="route-map-node" data-nav="agents" style="cursor:pointer">
      ...
      <text x="${n.x + 40}" y="${n.y + NODE_H / 2 - 4}" class="route-map-node-label">${escAttr(n.label)}</text>
      <text x="${n.x + 40}" y="${n.y + NODE_H / 2 + 12}" class="route-map-node-sub">${n.isDefault ? '★ ' + t('pages.route_map.defaultAgent') : n.id}</text>
    </g>`
  }
  ...
  for (let i = 0; i < a2aEdges.length; i++) {
    ...
    const mx = cx - 4
    const my = (y1 + y2) / 2
    svg += `<text x="${mx}" y="${my - 4}" text-anchor="end" class="route-map-edge-label" style="fill:#f59e0b">${t('pages.route_map.subAgentCall')}</text>`
  }
  ...
  svg += `<text x="${lx + 30}" y="${ly + 4}" class="route-map-edge-label" style="font-size:10px;fill:var(--text-secondary)">${t('pages.route_map.legendBinding')}</text>`
  lx += 110
  // Dashed line = default route
  svg += `<line x1="${lx}" y1="${ly}" x2="${lx + 24}" y2="${ly}" stroke="#94a3b8" stroke-width="2" stroke-dasharray="6,4"/>`
  svg += `<text x="${lx + 30}" y="${ly + 4}" class="route-map-edge-label" style="font-size:10px;fill:var(--text-secondary)">${t('pages.route_map.legendDefault')}</text>`
  if (a2aEdges.length > 0) {
    lx += 110
    svg += `<line x1="${lx}" y1="${ly}" x2="${lx + 24}" y2="${ly}" stroke="#f59e0b" stroke-width="1.5" stroke-dasharray="4,3"/>`
    svg += `<text x="${lx + 30}" y="${ly + 4}" class="route-map-edge-label" style="font-size:10px;fill:#f59e0b">${t('pages.route_map.subAgentCall')}</text>`
  }
  svg += `</g>`
