...
export function openAIDrawerWithError(errorCtx) {
  sessionStorage.setItem('assistant-error-context', JSON.stringify({
    ...
    title: errorCtx.title || t('comp_ai_drawer.error_operation_failed'),
    hint: errorCtx.hint || '',
  ...
  if (getCurrentRoute() !== '/assistant') {
    if (_fab?.el) {
      ...
      import('./toast.js')
        .then(({ toast }) => toast(t('comp_ai_drawer.error_context_saved'), 'info'))
        .catch(() => {})
    }
  } else {
...
function createFab() {
  ...
  fab.title = t('comp_ai_drawer.title')
...
function restorePosition(fab) {
  try {
    let raw = localStorage.getItem(POS_KEY)
    // Migration: check old key if new key absent
    if (!raw) {
      raw = localStorage.getItem('clawpanel-fab-pos')
      if (raw) localStorage.setItem(POS_KEY, raw)
    }
    if (!raw) return
...
function showDragHintOnce(el) {
  ...
  tip.textContent = t('comp_ai_drawer.drag_hint')
  el.appendChild(tip)
  localStorage.setItem(HINT_KEY, '1')
